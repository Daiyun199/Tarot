...
const DeleteSchedule: React.FC = () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (!isPastDay) {
        slots = Array.from({ length: 14 }, (_, idx) => {
          ...
          // Kiểm tra xem slot có được phép chọn hay không
          const isAvailable = disabledSlots.some(
            (slot) =>
              slot.dayOfWeek === format(date, "eeee") && // Kiểm tra ngày
              slot.startTime === `${hour.toString().padStart(2, "0")}:00:00` // Kiểm tra giờ
          );
          ...
          return {
            ...
            disabled: !isAvailable, // Nếu không có trong danh sách API, disable slot này
          };
        });
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (const slot of selectedSlots) {
        const response = await api.delete(`/ScheduleReader/${slot.id}`);
  ...
  const fetchSchedule = async () => {
    ...
    try {
      const response = await api.get(
        `ScheduleReader/schedule-not-booked?startDate=${startDate}&endDate=${endDate}`
      );
  ...
  const toggleSlot = (dayIdx: number, slotIdx: number) => {
    ...
    if (slot.selected) {
      ...
      // Tìm `id` của slot được chọn từ `disabledSlots` dựa trên ngày và giờ
      const disabledSlot = disabledSlots.find(
        (disabled) =>
          disabled.dayOfWeek === format(day.date, "eeee") &&
          disabled.startTime ===
            `${slot.hour.toString().padStart(2, "0")}:00:00`
      );
      ...
      setSelectedSlots((prevSelected) => [
        ...prevSelected,
        {
          ...
          hour: slot.hour,
          id: disabledSlot ? disabledSlot.id : "", // Thêm id vào selectedSlots
        },
      ]);
